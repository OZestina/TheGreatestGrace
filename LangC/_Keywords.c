...
static  // [지역변수]:함수 내 선언된 static 지역 변수
        // 전역변수의 성격을 지니는 변수가 된다. (선언방식) static int num(이 경우 0으로 초기화); static int num=0;
        // (특성1) 선언된 함수 내에서만 접근이 가능.
        // (특성2) 1회만 초기화. 초기화하지 않으면 0으로 초기화. (사실상 프로그램 시작과 동시에 할당 및 초기화 돼 변수를 선언한 반복문 내에서 초기화 값으로 다시 세팅되지 않음)
        // (특성3) 프로그램 종료 시까지 메모리 공간에 존재 (함수를 빠져나가도 소멸되지 않는다)
        
        // [전역변수]
        // 선언된 파일 내에서만 참조를 허용하겠다
        
        // [함수]
        // 함수에도 static 선언을 할 수 있다 (ex) static void MinCnt(void) {...}
        // 파일 내에서만 접근이 가능하도록 함수를 제한하는 방법
